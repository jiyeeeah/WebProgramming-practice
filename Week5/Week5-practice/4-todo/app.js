...
//UI 모듈 정의하기
const app = {
  todoList: document.getElementById("todoList"), //To-Do항목 리스트 엘리먼트
  form: document.querySelector("form"), //To-Do 항목 입력 폼 엘리먼트
  titleInput: document.getElementById("title"), //할일 제목 입력 필드 엘리먼트
  ...
  renderItem(item) {
    ...
    const titleText = document.createElement("div"); //할일 제목을 표시하는 div엘리먼트 생성
    titleText.textContent = `${item.title} - ${item.dueDate}`; //할일 제목과 마감 기한을 텍스트로 설정
    titleText.classList.add("title"); //CSS클래스 추가

    const toggleBtn = document.createElement("button"); //완료 여부를 토글하는 버튼 엘리먼트 생성
    toggleBtn.textContent = "✔︎"; //텍스트 설정
    toggleBtn.addEventListener("click", () => {
      //클릭 이벤트 핸들러 등록
      todoList.toggleComplete(item); //항목의 완료 여부를 토글
      this.renderList(); //To-Do 리스트 렌더링
    });
    ...
    removeBtn.addEventListener("click", () => {
      //클릭 이벤트 핸들러 등록
      todoList.remove(item); //항목 삭제, 여기의 todoList는 import해온 것 this.todoList는 이 객체 맨 처음에 데려온 애
      this.renderList(); //To-Do 항복 리스트 렌더링
    });
  ...
  //초기화 메소드
  init() {
    todoList.load(); //로컬 스토리지에서 To-Do 항목 리스트로 불러오기
    this.renderList(); //To-Do 항목 리스트 렌더링

    this.form.addEventListener("submit", (event) => {
      //폼 제출 이벤트 핸들러 등록
      event.preventDefault(); //기본 동작 취소
      const title = this.titleInput.value; //입력된 할일 제목
      const dueDate = this.dueDateInput.value; //입력된 마감 기한

      if(title && dueDate) {
        //할일 제목과 마감 기한이 모두 입력되었는지 확인
